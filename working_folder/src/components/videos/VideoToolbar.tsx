...
interface VideoToolbarProps {
  ...
  onSortChange: (order: VideoSortOrder) => void;
  onPublishClick?: () => void;
}
...
export const VideoToolbar: React.FC<VideoToolbarProps> = ({
  ...
  const handleSelectOrder = (order: VideoSortOrder) => {
    onSortChange(order);
    setIsMenuOpen(false);
  };
  ...
  return (
    <div className="flex flex-col gap-y-6 md:flex-row md:items-center md:justify-between">
      <div>
        <h1 className="text-3xl font-bold text-white">Watch Videos</h1>
        ...
        <div className="relative" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 z-20 mt-2 w-44 overflow-hidden rounded-lg border border-sky-800/70 bg-sky-950/95 shadow-lg">
              {(
                Object.keys(SORT_LABELS) as Array<VideoSortOrder>
              ).map((order) => (
                <button
                  key={order}
                  type="button"
